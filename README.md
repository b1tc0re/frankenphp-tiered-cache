...
`lastAccess` используется только для LRU-подобного eviction и обновляется при создании/замене записи, успешном `Get` и успешном `Touch`. Для expired/missing записей он не обновляется.
