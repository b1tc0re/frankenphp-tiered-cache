...
Background maintenance постепенно освобождает память от expired записей, к которым приложение больше не обращается. На каждом секундном maintenance tick выбирается один shard, shards обходятся по кругу. В выбранном shard просматривается не более 64 записей под `RLock`, поэтому параллельные `Get` продолжают выполняться. Scanner сохраняет позицию между посещениями shard и продолжает текущий map iteration вместо запуска нового случайного `range`, поэтому стабильные записи постепенно проходят полный bounded scan. Найденные expired candidates затем удаляются короткими `Lock` с повторной проверкой, что запись не была заменена конкурентным `Set`. Полная очистка expired записей перед eviction остаётся отдельным slow path и может пройти все shards, когда память нужно освободить немедленно.

Background maintenance не использует busy loop: одна goroutine просыпается по секундному ticker и выполняет ограниченный объём работы. Cleanup не добавляет per-request bookkeeping в `Get` или `Set`; состояние progressive scanner хранится на уровне shard.
